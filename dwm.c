...
struct Monitor {
	...
	int nmaster;
	int num;
    int btw;              /* width of of tasks portion of bar */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - (int)TEXTW(stext) - statusw + lrpad - 2) {
			x = selmon->ww - statusw;
			click = ClkStatusText;
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				//XMoveResizeWindow(dpy, m->barwin, m->wx, m->by, m->ww - 2 * sp, bh);
...
createmon(void)
{
	//Monitor *m;
	Monitor *m, *mon;
	int n, mi, max_bars = 2, istopbar = topbar;
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (m->tagset[m->seltags] & 1 << i)
			drw_rect(drw, x, bh - underline, w, underline, 1, 0);
